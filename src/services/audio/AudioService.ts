...
import { EMBEDDED_SAMPLE_LIBRARY } from './embeddedSamples';
...
export class ExpoSampleAudioService implements AudioService {
  ...
  private async initializeWebAudioFallback(): Promise<void> {
    ...
    const library = EMBEDDED_SAMPLE_LIBRARY[this.instrumentId] ?? EMBEDDED_SAMPLE_LIBRARY[DEFAULT_INSTRUMENT];
  ...
  private async loadInstrument(instrumentId: string): Promise<void> {
    const library = EMBEDDED_SAMPLE_LIBRARY[instrumentId] ?? EMBEDDED_SAMPLE_LIBRARY[DEFAULT_INSTRUMENT];
